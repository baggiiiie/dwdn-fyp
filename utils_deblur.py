...
def get_uperleft_denominator(img, kernel):
    ...
    output_fft = torch.fft.fft2(img1, dim=(-3, -2, -1))
    numerator = torch.stack((output_fft.real, output_fft.imag), -1)
...
def deconv(inv_ker_f, fft_input_blur):
    ...
    # output_deblur = torch.fft.ifft2(deblur_f, dim=(-3, -2, -1))
    deblur = torch.fft.ifft2(torch.complex(deblur_f[..., 0],
                                           deblur_f[..., 1]),
                             dim=(-3, -2, -1))
    deblur = deblur.real
...
def convert_psf2otf(ker, size):
    ...
    output_fft = torch.fft.fft2(psf, dim=(-3, -2, -1))
    otf = torch.stack((output_fft.real, output_fft.imag), -1)
    return otf
